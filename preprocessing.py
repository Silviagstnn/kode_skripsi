...
def rule_based(hasil_tokenisasi, top_5_similar):
    question_categories = {
    'apa': 'objek',
    'mengapa': 'alasan',
    'kapan': 'waktu',
        ...
        if sim > 0:
            ...
            for word in doc:
                if word in question_categories:
                    ...
                    categorized_docs[category].append((idx, doc))
                    break
...
def get_answer(cleaned_sentences, cosine_similarity_results, extract_pattern,relevant_docs):
    candidate_indices = [idx for idx, doc in relevant_docs]
    candidate_questions = [cleaned_sentences[idx] for idx in candidate_indices]
    input_user = ' '.join(cleaned_sentences[0])
    user_pattern, user_match = extract_pattern(input_user)
    previous_indices = [idx for idx, _ in cosine_similarity_results if idx in candidate_indices]
    result_texts = []
...
def hasil():
    if request.method == 'POST':
        ...
        relevant_docs = rule_based(cleaned_sentences, cosine_similarity_results) 
        # Membersihkan seluruh daftar kalimat dalam kolom 'Question'
        response = get_answer(cleaned_sentences, cosine_similarity_results, extract_pattern,relevant_docs)
